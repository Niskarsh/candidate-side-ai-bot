...
import { extractFromFreeText } from './extractor.js';
...
app.post('/api/extract', async (req, res) => {
    try {
      const { freeText, profile } = req.body || {};
      if (!freeText) return res.status(400).json({ ok: false, error: 'freeText is required' });
      const patch = await extractFromFreeText({ freeText, profile });
